...
public class HashMapOa<K, V> implements EvaluableMap<K, V> {
    ...
    public V remove(K key) {
        ...
        if(this.contains(key)) {
            while(DELETED.equals(table[position]) || !table[position].key.equals(key)) {
                position = calculatePosition(index, i++, key);
            }
            V val = table[position].value;
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        ...
        int position = index;
        if(this.contains(key)) {
            while(DELETED.equals(table[position]) || !table[position].key.equals(key) || !table[position].value.equals(oldValue)) {
                position = calculatePosition(index, i++, key);
            }
